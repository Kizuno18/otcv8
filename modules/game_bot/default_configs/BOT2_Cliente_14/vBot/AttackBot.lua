...
if not AttackBotConfig[panelName] or not AttackBotConfig[panelName][1] or #AttackBotConfig[panelName] ~= 3 then
  AttackBotConfig[panelName] = {
    [1] = {
      attackTable= {
        {
          description= "[7 Sqm] 1+ Any Creatures= Exori Hur, targeted (0%-100%)",
          mana= 1,
          orMore= true,
          monsters= true,
          category= 1,
    ...
    [2] = {
      attackTable= {
        {
          description= "[10 Sqm Range] 1+ Any Creatures= rune 3150, targeted (0%-100%)",
          mana= 1,
          orMore= true,
          monsters= true,
          category= 3,
        ...
        {
          description= "[10 Sqm Range] 1+ Any Creatures= rune 3155, targeted (0%-100%)",
          mana= 1,
          orMore= true,
          monsters= true,
          category= 3,
    ...
    [3] = {
      attackTable= {
        {
          description= "[7 Sqm] 1+ Any Creatures= Exori Con, targeted (0%-100%)",
          mana= 1,
          orMore= true,
          monsters= true,
          category= 1,
...
if not AttackBotConfig.currentBotProfile or AttackBotConfig.currentBotProfile == 0 or AttackBotConfig.currentBotProfile > 3 then 
...
function getPlayerVoc()
  ...
  onTextMessage(function(mode, text)
    ...
    elseif text:find("Paladin") then
      profile = 3
    elseif text:find("Sorcerer") or text:find("Druid") then
      profile = 2
    end

    if profile ~= nil then
      AttackBot.setActiveProfile(profile)
      ui.title:setOn(true)
      print("TEST")
    end
  end)
...
ui.settings.onClick = function(widget)
  ...
  for i=1,3 do
    ...
    AttackBot.getActiveProfile = function()
      return AttackBotConfig.currentBotProfile -- returns number 1-3
    end
  
    AttackBot.setActiveProfile = function(n)
      if not n or not tonumber(n) or n < 1 or n > 3 then
        return error("[AttackBot] wrong profile parameter! should be 1 to 3 is " .. n)
      else
