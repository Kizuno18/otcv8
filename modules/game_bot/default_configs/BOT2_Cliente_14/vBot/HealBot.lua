...
if not HealBotConfig[healPanelName] or not HealBotConfig[healPanelName][1] or #HealBotConfig[healPanelName] ~= 3 then
...
if not HealBotConfig.currentHealBotProfile or HealBotConfig.currentHealBotProfile == 0 or HealBotConfig.currentHealBotProfile > 3 then 
...
local setActiveProfile = function()
  ...
  --HealBotConfig[healPanelName][1].enabled = false
  --HealBotConfig[healPanelName][n].enabled = true
  print("Profile active: ", n)
...
function getPlayerVoc()
  ...
  onTextMessage(function(mode, text)
    ...
    if profile ~= nil then
      HealBot.setActiveProfile(profile)
      ui.title:setOn(true)
    end
  end)
end
...
if rootWidget then
  ...
  HealBot.getActiveProfile = function()
    return HealBotConfig.currentHealBotProfile -- returns number 1-3
  end

  HealBot.setActiveProfile = function(n)
    if not n or not tonumber(n) or n < 1 or n > 3 then
      return error("[HealBot] wrong profile parameter! should be 1 to 3 is " .. n)
    else
